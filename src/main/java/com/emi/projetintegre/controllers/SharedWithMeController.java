...
public class SharedWithMeController {
    ...
    public void initialize() {
        System.out.println("ShareWithMeController initialize: clientSocketManager=" + 
        ...
        recentPane.expandedProperty().addListener((_, _, newVal) -> {
        ...
        recentTable.focusedProperty().addListener((_, _, newVal) -> {
            if (newVal) {
                fileTable.getSelectionModel().clearSelection();
            }
        });
        fileTable.focusedProperty().addListener((_, _, newVal) -> {
            if (newVal) {
    ...
    private void setupTableColumns() {
        ...
        recentNameColumn.setCellFactory(_ -> {
    ...
    private void setupMenuColumn(TableColumn<PersonalDocument, Void> menuColumn) { //add click outside :)
    ...
    private void showFileActionMenu(PersonalDocument document, Button anchor) {
        try {
            ...
            if (resourceUrl == null) {
                ...
                showAlert("Error", "FileActionSharedWithMeMenu.fxml not found in resources.");
                return;
            }
    ...
    private void populateTables(String query) {
        ...
        ObservableList<PersonalDocument> fetchedDocuments = clientSocketManager.getListSharedWithMeDocuments(query);
        if (fetchedDocuments == null) {
            showAlert("Error", "Failed to fetch documents from server.");
            return;
        }
    ...
    private void handleSharedWithMeAction() {
        search.clear();
        populateTables(null);
        System.out.println("Share With Me action triggered");
    }
